...
use crate::{
    consts::{DHLEN, EMPTY_HASH, EMPTY_KEY, HASHLEN, MAC_LENGTH, NONCE_LENGTH, ZEROLEN},
    error::NoiseError,
...
impl CipherState {
    ...
    pub(crate) fn encrypt_with_ad(
        ...
        ad: &[u8],
        in_out: &mut [u8],
        mac: &mut [u8; MAC_LENGTH],
    ) -> Result<(), NoiseError> {
        let nonce = self.n.get_value()?;
        if self.has_key() {
            let mut ciphertext: Vec<u8> = in_out.to_vec();
            encrypt(
    ...
    pub(crate) fn decrypt_with_ad(
        ...
        let nonce = self.n.get_value()?;
        if self.has_key() {
            let mut plaintext: Vec<u8> = in_out.to_vec();
            if decrypt(
                from_slice_hashlen(&self.k.as_bytes()[..]),
                nonce,
                ad,
    ...
    pub(crate) fn write_message_regular(&mut self, in_out: &mut [u8]) -> Result<(), NoiseError> {
    ...
    pub(crate) fn read_message_regular(&mut self, in_out: &mut [u8]) -> Result<(), NoiseError> {
        let (in_out, mac) = in_out.split_at_mut(in_out.len() - MAC_LENGTH);
        let mut temp_mac: [u8; MAC_LENGTH] = [0u8; MAC_LENGTH];
...
impl SymmetricState {
    ...
    pub(crate) fn decrypt_and_hash(&mut self, in_out: &mut [u8]) -> Result<(), NoiseError> {
        ...
        temp_mac.copy_from_slice(mac);
        self.cs
            .decrypt_with_ad(&self.h.as_bytes()[..], ciphertext, &mut temp_mac)?;
...
impl HandshakeState {
    ...
    pub(crate) fn get_remote_static_public_key(&self) -> PublicKey {
        self.rs
    }
    ...
    pub(crate) fn write_message_b(
        &mut self,
        in_out: &mut [u8],
    ) -> Result<(Hash, CipherState, CipherState), NoiseError> {
        if in_out.len() < DHLEN {
            return Err(NoiseError::MissingneError);
        }
        if self.e.is_empty() {
